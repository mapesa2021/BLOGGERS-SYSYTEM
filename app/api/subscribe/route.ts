...
export async function POST(request: NextRequest) {
  try {
    ...
    } else if (templateType === 'business-services-pro') {
      // Business services pro template: requires creatorId, userId and phoneNumber (hardcoded)
      if (!body.creatorId || !body.userId || !body.phoneNumber) {
        return NextResponse.json({
          success: false,
          message: 'Missing required fields: creatorId, userId, phoneNumber'
        }, { status: 400 });
      }
    }
    ...
    if (templateType === 'minimal') {
      ...
      console.log('📝 Step 1: Smart user handling for JOSE template...');
      ...
      console.log('📝 Step 1: Smart user handling for business-services template...');
      ...
      console.log('📝 Step 1: Smart user handling for betting template...');
      ...
      // Business services pro template: use hardcoded creatorId and userId
      console.log('📝 Step 1: Using hardcoded creatorId and userId for business-services-pro template...');
      userId = parseInt(body.userId);
      creatorId = parseInt(body.creatorId);
      console.log('✅ Using hardcoded IDs - User ID:', userId, 'Creator ID:', creatorId, 'Phone:', body.phoneNumber);
    }
